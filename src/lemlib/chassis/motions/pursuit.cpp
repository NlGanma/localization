...
std::vector<lemlib::Pose> getData(const asset& path) {
    ...
    for (std::string line : dataLines) {
        ...
        try {
            ...
            lemlib::infoSink()->error("Failed to parse path file! Raw line: {}", stringToHex(line));
            break;
        }
 ...
 */
int findClosest(const lemlib::Pose& pose, const std::vector<lemlib::Pose>& path) {
...
lemlib::Pose lookaheadPoint(lemlib::Pose lastLookahead, lemlib::Pose pose, std::vector<lemlib::Pose> path, int closest,
                            float lookaheadDist) {
    if (path.size() < 2) return lastLookahead;
    ...
    for (int i = start; i < static_cast<int>(path.size()) - 1; i++) {
    ...
    for (int i = 0; i < timeout / 10 && pros::competition::get_status() == compState && this->motionRunning; i++) {
        ...
        if (targetVel <= 0.0f && pose.distance(pathPoints.back()) > endTolerance) {
            ...
            for (int idx = closestPoint + 1; idx <= finalPoint; ++idx) {
                ...
                targetVel = pathPoints.at(idx).theta;
                foundFutureSpeed = true;
                break;
            }
            if (!foundFutureSpeed) targetVel = prevVel;
        }
        ...
        if (forwards) {
            ...
            moveDrive(-targetRightVel, -targetLeftVel);
        }
