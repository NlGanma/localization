...
float lemlib::TrackingWheel::getDistanceTraveled() {
    if (this->encoder != nullptr) {
        ...
        for (int i = 0; i < this->motors->size(); i++) {
            // Skip invalid readings from disconnected motors
            if (std::isnan(positions[i]) || std::isinf(positions[i]) || std::fabs(positions[i]) > 1e6) continue;
            float in;
            switch (gearsets[i]) {
                ...
                default:
                    in = 200;
                    lemlib::infoSink()->warn("Unknown gearset on motor {}, defaulting to 200rpm", i);
                    break;
            }
...
float lemlib::TrackingWheel::getWheelDiameter() { return this->diameter; }

float lemlib::TrackingWheel::getGearRatio() { return this->gearRatio; }
